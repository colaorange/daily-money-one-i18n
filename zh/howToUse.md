...
程式允許您在不同帳本間的帳戶作轉換，不論是在基本或是進階的帳目編輯建立頁面，您可以在帳戶挑選時，選擇其他帳本的帳目。由於不同的帳本間的幣值可能不同，且當下的匯率不是固定，例如新台幣轉美金，所以在這個狀況下，程式不會限制你轉出的總金額必需等於轉入的總金額，請您在建立時依實際的金額來建立並且注意正確性。
...
只需在資產負債表的帳號項目上右滑，然後點選`建立預算`，選擇周期模式。在建立之後，即可在資產負債表或主畫面的快速檢示中看到預算的執行進度。此外，您可以在`預算管理`頁面中對這些項目進行排序、編輯或移除。